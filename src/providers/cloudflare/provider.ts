...
export class CloudflareProvider
  ...
  private validateDnsRecordDesiredState(
    ...
    this.ensureNonEmptyValue(desiredState.zoneId, 'Cloudflare DNS record zoneId');
    this.ensureNonEmptyValue(
      desiredState.recordName,
      'Cloudflare DNS record recordName',
    );
    this.ensureNonEmptyValue(desiredState.value, 'Cloudflare DNS record value');
  ...
  private validateTunnelDesiredState(
    ...
    this.ensureNonEmptyValue(desiredState.accountId, 'Cloudflare tunnel accountId');
    this.ensureNonEmptyValue(
      desiredState.tunnelName,
      'Cloudflare tunnel tunnelName',
    );
    this.ensureNonEmptyValue(desiredState.hostname, 'Cloudflare tunnel hostname');
    this.ensureNonEmptyValue(
  ...
  private validateAccessApplicationDesiredState(
    ...
    if (!('applicationName' in desiredState) || !('domain' in desiredState)) {
      throw new Error('Cloudflare Access application desired state is invalid.');
    }
  ...
  private validateAccessPolicyDesiredState(
    ...
    if (!('policyName' in desiredState) || !('allowedEmails' in desiredState)) {
      throw new Error('Cloudflare Access policy desired state is invalid.');
    }
  ...
  private hasAccessPolicyDifference(
    ...
    if (!('policyName' in actualState) || !('allowedEmails' in actualState)) {
      return true;
    }
    ...
    return (
      ...
      desiredState.allowedEmails.join(',') !== actualState.allowedEmails.join(',')
    );
  }
