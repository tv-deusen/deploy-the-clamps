	...
	implements Planner<ResourceGraph, ResourceGraph>
{
	...
	public async createPlanFromCompileResult(
		...
		return await this.createPlanFromGraph(
			input.compileResult.graph,
			context,
		);
	}
	...
	public async createPlanFromGraph(
		...
		const resourceGroups = this.groupResourcesByProvider(
	...
	private groupResourcesByProvider(
		...
		for (const resource of resources) {
			const providerName = this.mapCompilerProviderName(
				resource.provider,
			);
			const provider = this.providers.get(providerName);
