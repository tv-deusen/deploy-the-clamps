...
export class Compiler implements ConfigCompiler {
  ...
  private buildResources(input: BuildResourcesInput): BuildResourcesResult {
    ...
    if (input.networkDocument?.firewall) {
      ...
      resources.push({
        ...
        desired: {
          ...
          inboundRules: input.networkDocument.firewall.inboundRules.map(
            (rule) => ({
    ...
    if (input.networkDocument?.dns && input.providersDocument?.cloudflare) {
      ...
      for (const [index, record] of input.networkDocument.dns.records.entries()) {
        resources.push({
    ...
    if (input.tunnelDocument && input.providersDocument?.cloudflare) {
      ...
      resources.push({
        ...
        desired: {
          ...
          content: renderCloudflaredConfig(input.tunnelDocument, serviceUrl),
          mode: '0644',
        },
        dependsOn: baseDependsOn,
        tags: deploymentTags,
      ...
      resources.push({
        ...
        desired: {
          ...
          enabled: true,
          wantedState: 'active',
          unitFileContent: renderCloudflaredServiceUnit(input.tunnelDocument),
        },
        dependsOn: [
      ...
      if (input.tunnelDocument.access.enabled) {
        resources.push({
          ...
          desired: {
            accountId,
            applicationName: input.tunnelDocument.access.applicationName,
            domain: input.tunnelDocument.hostname,
            sessionDuration: input.tunnelDocument.access.sessionDuration,
          },
        ...
        resources.push({
          ...
          desired: {
            ...
            allowedEmails: [...input.tunnelDocument.access.allowedEmails],
            precedence: 1,
          },
