        ...
        .object({
          ...
          resources: z
            .object({
              ...
              cpuQuotaPercent: z.number().int().positive().optional(),
            })
            .strict()
            .optional(),
        })
...
const tunnelConfigSchema = configDocumentBaseSchema
  .extend({
    ...
    service: z
      .object({
        target: z.enum(['openclaw-gateway', 'url']).default('openclaw-gateway'),
    ...
    systemd: z
      .object({
        ...
        configPath: z.string().min(1).default('/etc/cloudflared/config.yml'),
      })
      .strict()
      .default({
  ...
  .superRefine((value, context) => {
    if (value.service.target === 'url' && !value.service.url) {
      context.addIssue({
        ...
        message: 'tunnel.service.url is required when service.target is "url".',
        path: ['service', 'url'],
      });
    }
  });
...
export class ZodConfigDocumentParser implements ConfigDocumentParser {
  public parseDocument(input: ParsedConfigDocument): ConfigDocument {
